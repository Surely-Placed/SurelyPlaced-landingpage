...
export function Programs() {
  const programs = [
    ...
    {
      id: "program-mentorship" as const,
    ...
    {
      id: "program-applications" as const,
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {programs.map((p, idx) => (
            <Reveal key={p.name} delay={idx * 80}>
              <article
                id={p.id}
                className="flex h-full scroll-mt-28 flex-col rounded-2xl border border-slate-200 bg-white/70 p-5 shadow-soft/40 transition-transform duration-300 hover:-translate-y-1.5 hover:shadow-elevated"
              >
